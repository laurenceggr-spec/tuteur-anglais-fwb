...
<div class="app-container">
    ...
    <div id="settings-modal" class="modal">
        ...
        <p style="font-size: 0.9rem;">Paste your Google Gemini API Key here to enable Smart Mode.</p>
        <input type="password" id="api-key-input" class="settings-input" placeholder="Paste API Key here...">
...
<script>
    ...
    window.onload = function() {
        ...
        if (apiKey && apiKey.startsWith("AIza")) {
            addMessage("System: AI Key loaded. Ready! 🧠", "ai");
    ...
    function saveSettings() {
        ...
        if (rawKey.length > 0 && !rawKey.startsWith("AIza")) {
            keyStatus.innerText = "⚠️ Key should start with 'AIza'.";
            return;
        }
        apiKey = rawKey;
    ...
    async function botReply(userText) {
        if (apiKey && apiKey.startsWith("AIza")) await callGeminiAPI(userText);
        else simpleBotReply(userText);
    }

    // --- CERVEAU AVEC CONTRAINTES ---
    async function callGeminiAPI(userText) {
        ...
        try {
            let aiReply;
            try { aiReply = await tryModel("gemini-1.5-flash"); } 
            catch (e) { console.warn("Flash failed, trying Pro"); aiReply = await tryModel("gemini-pro"); }
